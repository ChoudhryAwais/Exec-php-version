...
<?php
	...
	{
		$departmentId=$_POST['check'];
		require "connectdatabase.php";
		$DepartmentName = "SELECT DepartmentName,DepartmentCode,IsActive 
					  FROM department
					  WHERE DepartmentId='$departmentId'";
        
		$query1 = mysqli_query($conn,$DepartmentName);
		$department_Name = mysqli_fetch_array($query1);
		$department_actual_name = $department_Name['DepartmentName'];
		$department_actual_code = $department_Name['DepartmentCode'];
        $actual_isactive = $department_Name['IsActive'];
	}
	else
	{
	       					...
	       					<div class="col-xl-12 mt-5">
	       						<div class="container">
	       							<div class="container" style="font-size: 13px">
	       								<form class="form-group needs-validation" novalidate style="box-shadow: 1px 1px 10px;" action="viewdepartment.php" method="post">
	       									<table class="table table-bordered" style="line-height: 8px">
	       										...
	       										<tr>
	       											<td>
	       												<input onkeyup="spacefilter(this)" type="text" name="DepartmentCode" class="form-control text-uppercase form-control-sm" value="<?php echo $department_actual_code?>" required >
	       											</td>
	       											<?php
	       												if ($departmentId<=3) 
	       												{
			       											?>
				       											<td>
				       												<input style="cursor: no-drop;" onkeyup="numberfilter(this)" type="text" name="DepartmentNamedumy" class="form-control text-uppercase form-control-sm" value="<?php echo $department_actual_name?>" required disabled>
				       												<input style="cursor: no-drop;" type="hidden" name="DepartmentName" class="form-control text-uppercase form-control-sm" value="<?php echo $department_actual_name?>">
				       											</td>
			       											<?php
		       											}
		       											else
		       											{
		       												?>
				       											<td>
				       												<input onkeyup="numberfilter(this)" type="text" name="DepartmentName" class="form-control text-uppercase form-control-sm" value="<?php echo $department_actual_name?>" required >
				       											</td>
			       											<?php	
		       											}
	       											?>
	       									...
	       									<div class="row">
	       										<div class="col-xl-12 text-center p-2">
	       											...
	       											<input type="hidden" name="departmenteditcheck" value="true">
