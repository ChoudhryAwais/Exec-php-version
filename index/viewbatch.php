...
<?php
	...
	if (!empty($_POST['batcheditcheck'])) 
	{
		$batchNewName = htmlspecialchars(strtoupper($_POST['BatchName']));
		$batchId = $_POST['batchId'];
		$ActiveStatus =$_POST['ActiveStatus'];
		$i = 0;
		require "connectdatabase.php";

		$noofbatch = "SELECT COUNT(BatchId) as batchno 
				  	  FROM batch
				  	  WHERE (CampusId='$CampusId') AND BatchId != '$batchId'";
		$query1 = mysqli_query($conn,$noofbatch);
		$batchno = mysqli_fetch_array($query1);
		$no_Of_batch = $batchno['batchno'];

		$batchdata = "SELECT BatchName 
					  FROM batch
					  WHERE CampusId='$CampusId' AND BatchId != '$batchId'";
		...
		if (mysqli_num_rows($query2) > 0) 
		{
			...
			{
				if ($row['BatchName']==$batchNewName) 
				{
					echo 
						'<input type="hidden" >
						<script>
							Swal.fire("Error!", "This Batch Name Already Exist", "error");
						</script>
						';
				}
				else
		...
		if ($i==$no_Of_batch) 
		{
			$UpdateBatch = "UPDATE batch 
							SET BatchName='$batchNewName',IsActive = '$ActiveStatus' 
							WHERE BatchId = '$batchId'
							";
			if (mysqli_query($conn,$UpdateBatch)) 
			{
				echo 
					'<input type="hidden" >
					<script>
...
<?php
	...
	{
		$idofbatch=$_POST['check1'];
		require "connectdatabase.php";
		$deletebatch = "DELETE FROM Batch WHERE BatchId = '$idofbatch'";
		if (mysqli_query($conn,$deletebatch)) 
		{
			echo 
			'<input type="hidden" >
			<script>
...
<?php
	...
	$batchdata= "SELECT * 
                  ...
                  ";
	$query=mysqli_query($conn,$batchdata);
	$noofbatch = "SELECT COUNT(BatchId) as batchno 
				  FROM batch
				  WHERE CampusId='$CampusId'";
	$query1 = mysqli_query($conn,$noofbatch);
	       							...
	       							<div class="container table-responsive" style="font-size: 13px">
		       							<table class="text-center table-sm table table-bordered table-striped" style="box-shadow: 1px 1px 10px">
			       							...
			       							<?php
			       								...
			       								else
			       								{
			       									while($row = mysqli_fetch_assoc($query)) 
			       									{
			       										?>
			       											<tr>
				       											<td style="line-height: 25px" class="alert alert-primary"><?php echo $row['BatchName']?></td>
				     											...
				     											<td class="text-center p-2" style="line-height: 25px">
				       												<div class="btn-group text-center">
				       													<form action="batchedit.php" method="post">
											       							...
											       							<input type="hidden" name="check" value="<?php echo $row['BatchId']?>">
											       						</form>
											       						<a href="deletebatch.php?id=<?= $row['BatchId']?>" class = "btn-del"><button                                                                                          style="background-color: red;font-size: 11px" name="submit" class="btn-sm btn-danger                                                                                  rounded-circle" title="Delete"><i class="fa fa-trash"></i></button></a>
										       						</div>
										       					</td>
				       										</tr>
			       										<?php
			       							...
			       							<tr style="background-color: lightgray">
			       								<td class="border-0 font-italic text-left" style="font-size: 14px;">Total Record : <?php echo $no_Of_batch?></td>
