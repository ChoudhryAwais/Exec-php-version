...
<?php
	...
	{
		$subjectsId=$_POST['check'];
		$SubjectsName = "SELECT BoardsName,GroupsName,ClassesName,SubjectsName,IsActive 
					  FROM subjects
					  WHERE SubjectsId='$subjectsId'";
		$query1 = mysqli_query($conn,$SubjectsName);
		$Subjects_Name = mysqli_fetch_array($query1);
		$subjects_actual_name = $Subjects_Name['SubjectsName'];
		$boards_actual_name = $Subjects_Name['BoardsName'];
		$groups_actual_name = $Subjects_Name['GroupsName'];
		$classes_actual_name = $Subjects_Name['ClassesName'];
        $actual_isactive = $Subjects_Name['IsActive'];
	}
	else
	{
...
<?php
	...
	// for boards
	$boardsdata = "SELECT DISTINCT(BoardsName)
				 FROM boards
				 WHERE CampusId = '$CampusId' AND IsActive='Yes'
				 ORDER BY BoardsName ASC
				";
	$query5 = mysqli_query($conn,$boardsdata);
?>
	       				...
	       				<div class="row">
	       					...
	       					<div class="col-xl-9 col-lg-9 col-md-9 col-sm-9 text-right">
	       						<a href="viewsubjects.php"> 
	       							<button class="btn btn-sm btn-secondary mt-2 mr-4 text-white" style="border-radius: 40px;"><i class="fa fa-eye pr-1"></i>View                                          Subjects</button>
	       						</a>
	       					</div>
	       				</div>
	       				<div class="row">
	       					<div class="col-xl-12 mt-5">
	       						<div class="container">
	       							<div class="container" style="font-size: 13px">
	       								<form class="mb-5 form-group needs-validation" novalidate style="box-shadow: 1px 1px 10px;" action="viewsubjects.php"                                                   method="post">
	       									<table class="table table-bordered" style="line-height: 8px">
	       										...
	       										<tr>
	       											<td>
                                                <input onkeyup="spacefilter(this)" type="text" name="SubjectsName" class="form-control text-uppercase                                                                  form-control-sm" value="<?php echo $subjects_actual_name?>" required>
	       											</td>
	       											<td>
	       									...
	       									<?php  
	       										echo "<div class='row mt-5 pl-5' id='subj'>";
	       											...
	       											{
	       												if ($row['BoardsName']==$boards_actual_name) 
	       												{
	       													?>
			       												<div class="col-xl-3 col-md-4" style="font-size: 13px">
																	<input checked type="radio" id="boards<?php echo $i?>" name="boards" value="<?php echo $row['BoardsName']?>">
																	<label for="boards<?php echo $i?>"><?php echo $row['BoardsName']?></label>
																</div>
		       												<?php	
	       												}
	       												else
	       												{	
			       											?>
			       												<div class="col-xl-3 col-md-4" style="font-size: 13px">
																	<input type="radio" id="boards<?php echo $i?>" name="boards" value="<?php echo $row['BoardsName']?>">
																	<label for="boards<?php echo $i?>"><?php echo $row['BoardsName']?></label>
																</div>
			       											<?php
			       										}
	       									...
	       									<?php  
	       										echo "<div class='row mt-5 pl-5' id='subj'>";
	       											...
	       											{
	       												if ($row['GroupsName']==$groups_actual_name) 
	       												{
	       													?>
			       												<div class="col-xl-3 col-md-4" style="font-size: 13px">
	       									...
	       									<?php  
	       										echo "<div class='row mt-5 pl-5' id='subj'>";
	       											...
	       											{
	       												if ($row['ClassesName']==$classes_actual_name) 
	       												{
	       													?>
			       												<div class="col-xl-3 col-md-4" style="font-size: 13px">
																	<input checked type="radio" id="classes<?php echo $i?>" name="classes" value="<?php echo $row['ClassesName']?>">
	       									...
	       									<div class="row mt-5">
	       										<div class="col-xl-12 text-center p-2">
	       											...
	       											<input type="hidden" name="subjectseditcheck" value="true">
	       											<input type="hidden" name="subjectsId" value="<?php echo $subjectsId?>">
	       										</div>
	       									</div>
	       								</form>
	       							</div>
